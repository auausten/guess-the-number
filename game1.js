...
function getRandomNumber(max) {
    return Math.floor(Math.random() * max) + 1; // Generates random whole number from 1 to a maximum (set by input on line 24)
}
...
function startGame() {
    ...
    randomNum = getRandomNumber(10); // Sets 10 as the maximum
...
// Actual gameplay (shows results)
    if (userValue > 10 || userValue < 1 || userValue % 1 != 0) {
        result.textContent = 'Please insert a value between 1 and 10';
